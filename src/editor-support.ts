...
export const DSL_BUILTIN_DOCS: Record<string, DocEntry> = {
  ...
  setVariable: { sig: 'setVariable(name, value, props?)', detail: 'live-input row', info: 'One "setVariable" ROW (not a Table) — the same row in every event table with live inputs, so it reads the same whichever you route it to: hydra (a per-frame props value the sketch reads, no recompile), post (a keyframe track, `ease` shaping the segment into this one), bauble (a (def name value) compiled ahead of the sketch), particles (a sim parameter). props carries the other columns — beat (1-indexed), post\'s ease, hydra\'s out, disabled: rows([setCode("bloom((p) => p.glow)"), setVariable("glow", 0.35)]).outPost(). Omit beat inside an emit()/beatMap() template and the source row\'s timing places it, an Expr value baking against that row: paper.folds().beatMap(setVariable("blur", 8, { ease: "step" }), setVariable("blur", 0, { ease: "easeInOut" }), -1).outPost().' },
  setCode:    { sig: 'setCode(code, props?)',        detail: 'set-sketch row',   info: 'One "setCode" ROW (not a Table) — the same row in hydra, post and bauble: `code` replaces that view\'s whole sketch from this beat on. Only the language differs (a hydra chain / a post chain / a Janet shape expression), and in hydra the terminal .out() comes from props.out (default o0), so the code needn\'t write its own. props carries beat (1-indexed), out and disabled: rows([setCode("osc(20, 0.1, 1.2)"), setCode("noise(3)", { beat: 9 })]).outHydra().' },
  ...
  three:      { sig: 'three.box / .sphere / .light / .camera / .translate …', detail: 'three.js namespace', info: 'The three.js helpers, grouped under one namespace (shorthand: t). Scene primitives three.box/sphere/cylinder/cone/torus/text, the lights three.light/ambientLight/directionalLight/pointLight/spotLight/hemisphereLight, and the generic three.object(shape, props) each build a "create"-row Table — beat 1, at the origin — to concat and .rasterize(). three.points(shape, props) ⇄ three.geometry(points) sample a primitive to point rows and back. three.camera(keyframes) emits beat-timed camera moves from any table or rows, Expr-valued like a map(template). three.translate/scale/rotate(table, x, y, z) shift a scene table\'s create rows in place. e.g. t.box({ color: 0x4a9eff }).concat(t.sphere({ px: 1 })).rasterize(8).' },
  ...
  origami:    { sig: 'origami()', detail: 'folding paper', info: 'A sheet of paper folded by a table of fold steps, each solved exactly. Chain .steps(table) — one row per fold: p1/p2 two points "x,y" on the fold line (drawn on the current folded paper, unit-square frame), move sheet-space marker(s) for the flap(s) that swing, kind/pick to choose the move when a fold is ambiguous ("simple", "reverse", "sink", …), beat/dur/to its timing. A row with kind "turn" is a turn-over instead of a fold — its own step, no p1/p2/move. Then .spawn({ id, color, … }) for the paper itself — its mesh and every vertex, face and crease of it, beat-timed. Every row is verified: a fold that cannot lie flat fails with an error naming the step.' },
...
export const TABLE_METHOD_DOCS: Record<string, DocEntry> = {
  ...
  beatMap:     { sig: '.beatMap(beatRow, durRow?, durMult?)', detail: 'rows off beat/dur', info: 'Rebuild each row as one or two event rows placed off its own beat/dur, keeping the timing and discarding the rest — emit() for tables that are already beat-shaped, like .folds(). beatRow lands on the row\'s `beat`; durRow, when given, lands `dur` beats away, scaled by durMult (the default 1 = the end of the row\'s span, -1 = as far BEFORE the beat, 0.5 = halfway in). Both are emit() templates, so Expr values still read the source row — value: expr.field("plies").mul(0.2) — and a `beat` of their own overrides the placement. A row with no numeric `beat` starts at 1, one with no `dur` puts both rows on the same beat. e.g. paper.folds().beatMap({ event: "setVariable", name: "blur", value: 8, ease: "step" }, { event: "setVariable", name: "blur", value: 0, ease: "easeInOut" }, -1).outPost() snaps blur on with every fold and eases it off ahead of the next one.' },
  ...
  retime:      { sig: '.retime(timelineTable)',               detail: 'warp through timeline', info: 'Warp this table\'s `beat`s through a table of timeline events (schemas.timeline): each row lands at every playback beat its source beat is shown, so a "loop" event duplicates the looped rows once per cycle, a "retime" stretch rescales `dur` along with the spacing, and rows no event plays are dropped. Rows without a numeric `beat` — and every row when the timeline has no events — pass through unchanged. e.g. table("melody").retime(table("warp")).rasterize(). A mesh and its geometry are a baked animation rather than events, so they keep their own beats and are READ at the warped time instead — loop or stretch subsections of a folding with paper.spawn().retime(table("warp")).' },
  randomSegments: { sig: '.randomSegments({ count, vary, reverse, loop, seed })', detail: 'scrambled retime table', info: 'A timeline table (schemas.timeline) that cuts THIS table\'s beat span into `count` random-length chunks and deals them back out in a random order — every chunk played exactly once, filling the loop. The retime for CONTINUOUS content (a folding, a simulation, a motion path), where only the overall length matters and any source beat is a usable in-between. Feed it to .retime(), or route it with .outTimeline() to scramble the whole scene: fold.retime(fold.randomSegments({ count: 6, reverse: 0.3 })).outThree(). `count` how many chunks (default 8); `vary` how uneven their lengths are — 0 all equal, 1 anywhere from a sliver to double (default 0.5); `reverse` the chance a chunk runs backwards (default 0). Playback time is shared out in proportion, so with `loop` (default the GUI "beats" control) set to the span\'s own length every chunk plays at natural speed. The cut is re-rolled on every run — pass `seed` to pin one.' },
  ...
  steps:       { sig: '.steps(table | rows)',                 detail: 'origami fold table', info: 'One table = the whole folding, one row per fold, applied in order. p1/p2: two points "x,y" on the fold line, in the fixed unit-square frame the flat sheet started in. move: sample point(s) on the UNFOLDED sheet (";"-separated), one inside each flap that swings — a sheet point names exactly one ply, so single layers of a stack can be picked out. kind: "simple", "reverse", "sink", … chooses the move when several layer orders are valid; pick indexes among same-kind states. kind "crease" cuts every ply along the line WITHOUT folding (origami pre-creasing) — it takes no timeline slot and gives later folds and the soft motion extra bend lines. beat/dur: when the swing starts and how long it lasts, in beats (defaults: the row\'s position, 0.75). to: how far to swing — 1 lands flat; only the last row may stop short (wings held half-raised). kind "turn" makes the row a TURN-OVER rather than a fold: the model rotates onto its other face, creasing nothing, so the row needs no p1/p2/move — just beat/dur, its own step with its own timing. It is a diagram\'s "turn over", and how a fold that would work the underside is kept swinging toward the viewer.' },
  spawn:       { sig: '.spawn({ id, color, px, … })',         detail: 'origami create row', info: 'The origami sheet\'s scene create row. The paper arrives as a MESH (shape: "mesh") — faces, creases and vertex positions baked across the folding — not as a fold program the renderer has to know how to run, so the scene table speaks the same element vocabulary as .verts()/.faces()/.edges(). `fold` says where to read it (0 = flat sheet). Props merge over defaults. Colouring is NOT set here — it is its own pipeline off .verts()/.faces()/.edges().' },
  folds:       { sig: '.folds()',                             detail: 'origami fold table',  info: 'One row per fold, carrying that fold\'s beat/dur — so it is already an event: add an id and event and route it anywhere. Columns: step/name which fold, to how far it swings, kind its classification ("Pureland", "Inside Reverse", …), states how many valid layer orders existed, faces/moving/flaps how much paper there is and how much swings, layers the stack height, plies the deepest pile, motion how the in-between plays ("rigid", "relaxed", "mechanism", or "turn" for a turn-over row, whose kind is "Turn"). e.g. paper.folds().derive({ event: "pulse", name: "glow", value: 0.8 }).outPost() pulses a post-processing glow on every fold as it happens.' },
  verts:       { sig: '.verts({ at })',                       detail: 'origami corner table', info: 'What the folding knows about each corner, one row per (fold, vertex), carrying that fold\'s beat/dur: step/name which fold, vert which corner, hinge whether it lies ON the fold line (which is exactly why the paper can turn there without tearing), moving whether any face meeting there swings, faces how many meet there, cx/cy where it is drawn, sheetX/sheetY where it sits on the unfolded square, the one place it never moves. A vert number means the same corner at every step, so `vert` joins across folds. A scene "color" row carrying `vert` tints the paper at that corner, and a corner\'s colour beats its face\'s, so a few painted vertices shade across a face. `at` adds px/py/pz as in .faces().' },
  faces:       { sig: '.faces({ at })',                       detail: 'origami face table',  info: 'What the folding knows about each face, one row per (fold, face), carrying that fold\'s beat/dur: step/name which fold, face which face, moving whether it swings, flap which rigid flap it belongs to (-1 if still), dir the solver\'s rotation sense, layer/layerFrom its stacking RANK after/before (a rank, not a count), plies how many sheets are stacked where it lies (this is "how many layers"), back whether the paper\'s reverse shows, cx/cy/area as displayed, sheetX/sheetY on the unfolded square. A scene "color" row carrying `face` recolours that one face, so painting is an ordinary pipeline: paper.faces().filter({ moving: true }).derive({ id: "crane", event: "color", color: 0x2f6fff }).outThree(). `at` samples where each face IS at that fraction of its own fold (0 starting, 1 landed), adding px/py/pz in the paper\'s frame — point a camera at the moving flap with tx/ty/tz. A face number means the same paper at every step, so `face` joins across folds.' },
  edges:       { sig: '.edges({ at })',                       detail: 'origami crease table', info: 'What the folding knows about each crease, one row per (fold, edge), carrying that fold\'s beat/dur: step/name which fold, edge its number, a/b the vertices it joins, folds whether this fold actually TURNS this crease (the real "edges the fold is on" — being on the fold line is a different, wrong test: the line also cuts creases it merely crosses, and a reverse fold opens a spine crease nowhere near it), mv "M"/"V" once landed, hinge whether it separates moving paper from still, border whether it is the sheet\'s rim. A scene "color" row carrying `edge` recolours that one crease. `at` adds px/py/pz as in .faces().' },
