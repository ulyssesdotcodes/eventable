...
export const TABLE_METHOD_DOCS: Record<string, DocEntry> = {
  ...
  retime:      { sig: '.retime(timelineTable)',               detail: 'warp through timeline', info: 'Warp this table\'s `beat`s through a table of timeline events (schemas.timeline): each row lands at every playback beat its source beat is shown, so a "loop" event duplicates the looped rows once per cycle, a "retime" stretch rescales `dur` along with the spacing, and rows no event plays are dropped. Rows without a numeric `beat` — and every row when the timeline has no events — pass through unchanged. e.g. table("melody").retime(table("warp")).rasterize(). A mesh and its geometry are a baked animation rather than events, so they keep their own beats and are READ at the warped time instead — loop or stretch subsections of a folding with paper.spawn().retime(table("warp")). Paint — the "color" rows addressed to a mesh — is a channel of its own, and what you retime it with picks its clock: warp it WITH the paper (concat first, so the mesh\'s warp rides the colour rows too) and it is welded to the folding, read at the same source beat the geometry is; retime it separately and it stays a set of EVENTS moved onto playback time, free to run on its own clock or none. Which paint holds is a step function of SOURCE time, though, and moving its edges only rebuilds that where the warp keeps the order (a stretch, a hold, a speed) — a warp that REORDERS (randomSegments) sends a paint row and its release row to scattered playback beats and the colour holds across everything between them, so warp that one with the paper.' },
  ...
  faces:       { sig: '.faces({ at })',                       detail: 'origami face table',  info: 'What the folding knows about each face, one row per (fold, face), carrying that fold\'s beat/dur: step/name which fold, face which face, moving whether it swings, flap which rigid flap it belongs to (-1 if still), dir the solver\'s rotation sense, layer/layerFrom its stacking RANK after/before (a rank, not a count), plies how many sheets are stacked where it lies (this is "how many layers"), back whether the paper\'s reverse shows, cx/cy/area as displayed, sheetX/sheetY on the unfolded square. A scene "color" row carrying `face` recolours that one face, so painting is an ordinary pipeline. A colour row is a hard SWITCH that holds, so paint lasting only as long as the swing is two rows — one to paint, one at `beat + dur` handing the face back (color: null = unpainted): paper.faces().filter({ moving: true }).emit([{ id: "crane", event: "color", face: expr.field("face"), beat: expr.field("beat"), color: 0x2f6fff }, { id: "crane", event: "color", face: expr.field("face"), beat: expr.field("beat").add(expr.field("dur")), color: null }]).outThree(). `at` samples where each face IS at that fraction of its own fold (0 starting, 1 landed), adding px/py/pz in the paper\'s frame — point a camera at the moving flap with tx/ty/tz. A face number means the same paper at every step, so `face` joins across folds.' },
