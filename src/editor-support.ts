...
export const TABLE_METHOD_DOCS: Record<string, DocEntry> = {
  ...
  spawn:       { sig: '.spawn({ id, color, faces, … })',      detail: 'origami create row', info: 'The origami sheet\'s scene create row (shape: "origami"), with the compiled fold program riding along and fold at 0 (flat sheet). Props merge over defaults. faces/edges take painted rows from .faces()/.edges() — any row carrying a `color` recolours that element while its fold is on screen, and a `fade` of 1 makes the paint swell with the swing.' },
  faces:       { sig: '.faces()',                             detail: 'origami face table',  info: 'What the folding knows about each face, one row per (step, face): step/name which fold, face its number within that step, moving whether it swings, flap which rigid flap it belongs to (-1 if still), dir the solver\'s rotation sense, layer/layerFrom its stacking RANK after/before (a rank, not a count), plies how many sheets are stacked where it lies (this is "how many layers"), back whether the paper\'s reverse shows, cx/cy/area as displayed, sheetX/sheetY on the unfolded square. An ordinary table — filter/derive/save/graph it. Give rows a `color` (and optional `fade` 0…1) and pass them to spawn({ faces }) to paint. Face numbers only mean something inside their own step: every fold re-splits the paper.' },
  edges:       { sig: '.edges()',                             detail: 'origami crease table', info: 'What the folding knows about each crease, one row per (step, edge): step/name which fold, edge its number, a/b the vertices it joins, folds whether this fold actually TURNS this crease (the real "edges the fold is on" — being on the fold line is a different, wrong test: the line also cuts creases it merely crosses, and a reverse fold opens a spine crease nowhere near it), mv "M"/"V" once landed, hinge whether it separates moving paper from still, border whether it is the sheet\'s rim. Give rows a `color` and pass them to spawn({ edges }) to light creases up as they are made.' },
