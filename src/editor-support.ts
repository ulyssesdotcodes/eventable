...
export const TABLE_METHOD_DOCS: Record<string, DocEntry> = {
  ...
  sequence:    { sig: '.sequence(steps?)',                    detail: 'origami schedule',  info: 'DEAD, kept only so an old program still runs: the fold table\'s timings as keyframes driving a `fold` column that nothing reads any more. The paper\'s geometry comes from .spawn(), and a folding is retimed by warping the paper itself — paper.spawn().retime(table("warp")).' },
