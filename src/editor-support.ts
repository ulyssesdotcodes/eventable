...
export const DSL_BUILTIN_DOCS: Record<string, DocEntry> = {
  ...
  row:        { sig: 'row.setVariable / .setCode / .camera', detail: 'row builders', info: 'The builders that make ONE plain row instead of a Table, so the result is data you put IN a table — rows([…]), .concat() — and equally a template shape for emit()/beatMap()/derive(). row.setVariable(name, value, props?) and row.setCode(code, props?) write the event row every event table reads: the SAME row in hydra (a per-frame props value the sketch reads, no recompile), post (a keyframe track, `ease` shaping the segment into this one), bauble (a (def name value) compiled ahead of the sketch), and — setVariable only — particles (a sim parameter); props carries beat (1-indexed), post\'s ease, hydra\'s out, disabled. row.camera(props?) is one camera keyframe as a scene row (shape "camera"): a "create" seeds the whole pose (eye at 0,0,5, target the origin), event: "update" makes it a later keyframe carrying only the fields it moves. e.g. rows([row.setCode("bloom((p) => p.glow)"), row.setVariable("glow", 0.35)]).outPost(). Omit beat in an emit()/beatMap() template and the source row\'s timing places it, an Expr value baking against that row.' },
