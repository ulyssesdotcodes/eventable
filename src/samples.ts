...
rows([setCode("bloom((p) => p.glow, 0.5, 0.7)"), setVariable("glow", 0)])
...
//
// The 22 steps — 17 folds and the 5 turn-overs between them — are seeded into
// the "origami" table on the right; the code here declares only their column
// schema, so edit the steps live in the panel. Each column:
...
//          checks the paper really can fold that way. "turn" makes the row
//          a TURN-OVER rather than a fold: the model rotates onto its other
//          face and nothing is creased, so the row needs no p1/p2/move —
//          just a beat and dur, like any other step. The paper lies on a
//          table and you are looking down at it, so a fold that would work
//          the underside gets a turn row in front of it and swings toward
//          you instead of dipping away behind the model. Delete one (try
//          "overturn") and watch the fold after it go the wrong way.
...
// the folding turns over in the five places the diagram says to — each its
// own "turn" row, with its own beat, rather than something the engine
// guesses at. The fold value is one number: how many steps have landed —
// turn-overs count as steps, and fractions are the next one mid-swing — so
...
//     swing through.
//   - Give "overturn" dur: 4 and watch the model roll over slowly — a
//     turn-over is a step like any other, so it has its own timing.
//   - Delete "overturn" and the fold after it swings away from you behind
//     the model instead of toward you: that fold works the underside, and
//     the turn row is what brings that side up.
//
...
// pingpong the lotus: its folding runs over source beats 1–18 (twelve folds
// and the five turn-overs between them); the pingpong's until-next window
// (beat 1, closed by the hold at beat 35) plays that forward then backward
// across beats 1–35, so it blooms by beat 18 and is flat again by 35.
// .retime warps when the paper is read, not where its rows sit.
const bloomFall = rows([
  { event: "pingpong", beat: 1, from: 1, to: 18 },
  { event: "hold", beat: 35, from: 1 },
])
...
lotus.spawn({ id: "flowerA", color: 0xfff2d6, backColor: 0xe0518a, ...pose })
  ...
  .concat(rows([{ id: "flowerA", event: "destroy", beat: 35 }]))
  .concat(lily.spawn({ id: "flowerB", color: 0xf0eeff, backColor: 0x7a3fc0, ...pose }).shift(34))
  .outThree()
...
//   - pingpong the lily too — \`.retime(rows([{ event: "pingpong", beat: 35,
//     from: 1, to: 13 }]))\` in place of \`.shift(34)\` — so it opens and closes
...
// setCode(code, props) and setVariable(name, value, props) write these same
// two rows for you — the same pair in post and bauble too. Routing
...
rows([
  setCode("osc((props) => props.freq, 0.06, " + (0.4 + heat).toFixed(2) + ")" +
          ".kaleid(" + (3 + (n % 7)) + ")" +
          ".rotate((props) => props.time * " + (0.03 + 0.4 * heat).toFixed(3) + ")" +
          ".out(o0)"),
  setVariable("freq", 4 + (n % 24)),
]).outHydra()
...
rows([setCode("src(s0).blend(src(o0).scale(1.004), 0.2).out(o0)")]).outHydra()
